...
//SET VAR SET CITYHISTORY STORAGE
...
fetch (
     'http://api.openweathermap.org/data/2.5/uvi?lat=' + data.coord.lat + '&lon=' + data.coord.lon + '&units=imperial&appid=a4dc6b4797cfe0a360daceabbd77f8dc'
)
for (i = 9; i < 18; i++) {
    let textArea = $("#" + i); 
  // CHECK FOR CONDITION ONE/IF TRUE LINK TO CSS PAST
     if (i < hour) { 
    $("#uv-display-styles").css("background-color", "green");
     }
